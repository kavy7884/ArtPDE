...
int main() {
    ...
    auto ptr_view_pt = PositionVector<Dim3>::createViewPoint({1.0,2.0,3.0});
    decltype(PositionVector<Dim3>::createViewPoint()) ptr_view_pt_1;
    ptr_view_pt_1 = ptr_view_pt; //shallow copy

    std::cout << *ptr_view_pt <<  std::endl;
    std::cout << *ptr_view_pt_1 <<  std::endl;

    auto ptr_compute_pt = PositionVector<Dim3>::createComputePoint();
    auto ptr_compute_pt_1 = PositionVector<Dim3>::createComputePoint();
    auto ptr_compute_pt_2 = PositionVector<Dim3>::createComputePoint();
    ...
    ptr_compute_pt_2->addDataByList({1.0, 1.0, 1.0});

    std::cout << *ptr_compute_pt <<  std::endl;
    std::cout << *ptr_compute_pt_2 <<  std::endl;

    auto ptr_view_pt_2 = PositionVector<Dim3>::createViewPoint({9.0, 9.0, 9.0});
