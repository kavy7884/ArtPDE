...
namespace art_pde{
	...
	public:
			...
			evaluate_shape(const PointType& iso_point) override
		{
			...
			const double& eta = iso_point.getY();
			const double& zeta = iso_point.getZ();
			N_[0] = 0.125*(1. - xi)*(1. - eta)*(1. - zeta);
			N_[1] = 0.125*(1. + xi)*(1. - eta)*(1. - zeta);
			N_[2] = 0.125*(1. + xi)*(1. + eta)*(1. - zeta);
			N_[3] = 0.125*(1. - xi)*(1. + eta)*(1. - zeta);
			N_[4] = 0.5*(1. + zeta);
			...
			evaluate_Jacobian(const PointType& iso_point, const std::vector<PointType>& elem_nodes) override
		{
			...
			Jacobian_[0][1] = (1 / 8.)*((y2 - y1)*(eta - 1)*(zeta - 1) + (y4 - y3)*(eta + 1)*(zeta - 1));
			Jacobian_[0][2] = (1 / 8.)*((z2 - z1)*(eta - 1)*(zeta - 1) + (z4 - z3)*(eta + 1)*(zeta - 1));
			Jacobian_[1][0] = (1 / 8.)*((x2 - x3)*(xi + 1)*(zeta - 1) + (x4 - x1)*(xi - 1)*(zeta - 1));
			Jacobian_[1][1] = (1 / 8.)*((y2 - y3)*(xi + 1)*(zeta - 1) + (y4 - y1)*(xi - 1)*(zeta - 1));
			Jacobian_[1][2] = (1 / 8.)*((z2 - z3)*(xi + 1)*(zeta - 1) + (z4 - z1)*(xi - 1)*(zeta - 1));
			Jacobian_[2][0] = (1 / 2.)*x5 + (1 / 8.) * (x2*(eta - 1)*(xi + 1) - x1*(eta - 1)*(xi - 1) - x3*(eta + 1)*(xi + 1) + x4*(eta + 1)*(xi - 1));
			Jacobian_[2][1] = (1 / 2.)*y5 + (1 / 8.) * (y2*(eta - 1)*(xi + 1) - y1*(eta - 1)*(xi - 1) - y3*(eta + 1)*(xi + 1) + y4*(eta + 1)*(xi - 1));
